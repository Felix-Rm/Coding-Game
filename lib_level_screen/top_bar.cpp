...
TopBar::TopBar(LevelScreen *parent_window) : Drawable({}, {})
{
    ...
    this->time_text = sf::Text("00:00s", game_font, text_size);
    this->time_text.setFillColor(game_colors::GOLD);
    ...
    this->speed_text = sf::Text("100%", game_font, text_size);
    this->speed_text.setFillColor(game_colors::GOLD);
    this->speed_text.setStyle(sf::Text::Bold);

    //

    this->btn_play = new Button(this->window,
                                ...
                                "p", this->text_size * 0.8, 1, game_colors::WHITE, game_colors::GOLD, Window::createEventHandler(Window::noop, nullptr));
    this->btn_play->center();

    this->btn_stop = new Button(this->window,
                                {(float)(this->pos.x + this->size.x * (4.5 / 9.0) + this->size.x * (1 / 9)), (float)(this->pos.y + this->size.y * 0.5)},
                                {(float)(this->size.y * 0.8), (float)(this->size.y * 0.8)},
                                "s", this->text_size * 0.8, 1, game_colors::WHITE, game_colors::GOLD, Window::createEventHandler(Window::noop, nullptr));
    this->btn_stop->center();

    this->btn_exit = new Button(this->window,
                                {(float)(this->pos.x + this->size.x * (5.5 / 9.0) + this->size.x * (1 / 9)), (float)(this->pos.y + this->size.y * 0.5)},
                                {(float)(this->size.y * 0.8), (float)(this->size.y * 0.8)},
                                "e", this->text_size * 0.8, 1, game_colors::WHITE, game_colors::GOLD, Window::createEventHandler([](sf::Event &event, void *data) {
                                    ((Window *)data)->close();
                                    return true;
                                },
                                                                                                                                 this->window));
    ...
    this->btn_speed_plus = new Button(this->window,
                                      ...
                                      "+", this->text_size * (1 / 3.0), 1, game_colors::WHITE, game_colors::GOLD, Window::createEventHandler(Window::noop, nullptr));
    this->btn_speed_plus->center();

    this->btn_speed_minus = new Button(this->window,
